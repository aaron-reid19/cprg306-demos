    ...
    return(
        <main>
            ...
            <section className="mx-8 my-4 p-4 bg-emerald-500 dark:bg-emerald-800 md:bg-purple-300 dark:md:bg-purple-800 rounded-xl">
              ...
              <p>Put simpleArr values onto the page</p>
              <ul className="my-2">
                {simpleArr.map((v) => (
                  <li key={v}>{v}</li>
                ))}
            ...
            <section className="mx-8 my-4 bg-pink-300 dark:bg-pink-800 p-4 rounded-xl">
              <h2 className="text-xl sm:text-2xl font-bold"> key values need to be unique</h2>
              <p>even though index can technically be the same number, it'll change based on array changes.</p>
              <ol className="list-decimal ml-6 mt-4">
                {numArrObj.map((num) => (
                  <li key={num.id} className="text-xl font-bold">{num.value}</li>
                ))}
              </ol>
            </section>
            <section className="mx-8 my-4">
                <h2 className="text-2xl">Method Chaining & Using Return in Map </h2>
                <p> Chain methods like filter and map to get powerful rendering possibilities.</p>
                <p>If you use cury brackets, use a return statement aswell</p>
                <ul className="my-2">
                  {cats 
